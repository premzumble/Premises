-- Baseline Database Schema for Premises (PostgreSQL)
-- Version: 1.0.0
-- Description: Multi-tenant database design with strict referential integrity, indexes, and constraints.
...
CREATE TABLE organizations (
    ...
    type VARCHAR(50) NOT NULL, -- e.g., 'UNIVERSITY', 'COLLEGE', 'CORPORATE'
    domain TEXT UNIQUE, -- Nullable, e.g., 'stanford.edu'
    logo_url TEXT,
    website TEXT,
    address TEXT,
    ...
    -- Composite unique key to support composite foreign keys in child tables
...
CREATE TABLE departments (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Ensure department names are unique within the organization
    CONSTRAINT uniq_org_dept_name UNIQUE (organization_id, name),
    -- Supporting composite reference
...
CREATE TABLE admins (
    ...
    email VARCHAR(255) NOT NULL UNIQUE, -- Globally unique email for login simplicity
...
CREATE TABLE faculty (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Enforce department belongs to the same organization
...
CREATE TABLE devices (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Enforce faculty belongs to the same organization
...
CREATE TABLE location_events (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Ensure related record and faculty belong to the same organization
...
CREATE TABLE reason_requests (
    ...
    faculty_id UUID NOT NULL,
    attendance_record_id UUID REFERENCES attendance_records(id) ON DELETE CASCADE,
    reason_type VARCHAR(50) NOT NULL, -- e.g., 'OUTSIDE_GEOFENCE', 'LATE_ENTRY', 'EARLY_EXIT'
    notes TEXT,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Ensure related record, faculty and admin reviewer belong to the same organization
...
CREATE TABLE notifications (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Ensure faculty belongs to the same organization
...
CREATE TABLE audit_logs (
    ...
    actor_id UUID, -- Polymorphic reference, not strictly bound by FK to allow historic log preservation
    action TEXT NOT NULL,
    entity_type VARCHAR(50) NOT NULL, -- e.g., 'ORGANIZATION_SETTINGS', 'GEOFENCE', 'ATTENDANCE_POLICY'
    entity_id UUID,
    old_value JSONB,
    new_value JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE faculty_registration_requests (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Ensure faculty and reviewer belong to the same organization
...
CREATE TABLE device_change_requests (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Ensure faculty, device, and reviewer belong to the same organization
...
CREATE TABLE policy_change_history (
    ...
    -- Ensure policy and administrator belong to the same organization
